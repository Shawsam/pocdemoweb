...
<script setup>
  ...
  const emits = defineEmits(['itemSelect'])
  ...
  let scale = 400
  let animationFrame, tooltips, projection, path, svg, countries
  ...
  onMounted(() => {
    ...
    d3.json('https://toscdn.tarsocial.com/tarsocial/pocdemoweb/develop/world.json').then((data) => {
      ...
      //   emits('itemSelect', d.properties.index)
      // })
  ...
  const updateRender = (markers) => {
    ...
    svg
      ...
      .each(function (d) {
        ...
        if (_rotation >= 270) {
          //350
          if ((lon > 0 && lon < 450 - _rotation) || (lon < 0 && lon > _rotation - 270)) {
            visibility = true
          }
  ...
  const addMarker = () => {
    svg
      ...
      .attr('cy', (d) => projection([d.coordinates[0], d.coordinates[1]])[1])
      .attr('r', (d) => {
        return d.name ? markerR : 4
      })
      .attr('fill', (d) => {
        return d.name ? 'red' : 'coral'
      })
      .attr('cursor', 'pointer')
        ...
        tooltips
          ...
          .html(
            `<div style="display:flex; align-items:start; flex-direction:column">
              <div style="font-weight:bold">${d.group}</div>
              <div style="display:flex; align-items:start">
              <img style="height:50px; width:50px; margin-right:5px" class="icon" src="data:image/png;base64," />
              <div>
                <div>${d.gender}</div>
                <div>${d.age}</div>
                <div>${d.city}</div>
